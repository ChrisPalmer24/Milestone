...
export default function Track() {
  ...
  const [formState, setFormState] = useState({
    retirementAge: fireSettings?.targetRetirementAge || defaultSettings.targetRetirementAge,
    targetAmount: fireSettings ? 
      Number(fireSettings.annualIncomeGoal) * (100 / Number(fireSettings.safeWithdrawalRate)) : 
      defaultSettings.annualIncomeGoal * (100 / defaultSettings.safeWithdrawalRate),
  ...
  const handleRecalculate = async () => {
    ...
    try {
      await updateFireSettings({
        ...
        annualIncomeGoal: formState.targetAmount * (Number(fireSettings.safeWithdrawalRate) / 100),
    ...
    <div className="track-screen max-w-5xl mx-auto px-4 pb-20">
      <Card className="mt-4">
        <CardContent className="p-4">
          ...
          <p className="text-sm text-gray-600 mb-6">See if you're on track to reach your retirement goals</p>
              ...
              <div>
                ...
                <div className="flex items-center">
                  <span className="text-sm text-gray-600">
                    £{totalPortfolioValue.toLocaleString()} of £{targetAmount.toLocaleString()}
                  </span>
                </div>
              </div>
            </div>
          ...
          <div className="bg-gray-50 rounded-lg p-4 mb-6">
            ...
            <div className="flex justify-between items-center mb-1">
              <span className="text-sm text-gray-600">Expected by now (age {currentAge}):</span>
              <span className="font-medium">£{Math.round(onTrackStatus.expectedCurrentAmount).toLocaleString()}</span>
            </div>
            <div className="flex justify-between items-center mb-1">
              <span className="text-sm text-gray-600">Difference:</span>
              <span className={`font-medium ${onTrackStatus.isOnTrack ? 'text-secondary' : 'text-error'}`}>
                {onTrackStatus.difference >= 0 ? '+' : '-'}£{Math.abs(Math.round(onTrackStatus.difference)).toLocaleString()}
              </span>
            </div>
            ...
            {!onTrackStatus.isOnTrack && (
              <div className="mt-3 pt-3 border-t border-gray-200">
                <p className="text-sm">
                  To stay on track, consider increasing your monthly investment by{' '}
                  <span className="font-medium">£{monthlyAdjustment}</span>
                </p>
              </div>
            )}
          </div>
