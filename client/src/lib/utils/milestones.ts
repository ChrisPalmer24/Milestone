  ...
  nextMilestones.forEach(milestone => {
    ...
    if (!existingTargets.has(key)) {
      suggestions.push({
        ...
        description: `Grow your overall investments to £${formatNumberShort(milestone)}.`,
        icon: '💰'
      });
    }
  });
      ...
      if (accountNextMilestones.length > 0) {
        ...
        if (!existingTargets.has(key)) {
          suggestions.push({
            ...
            description: `Grow your ${type} account to £${formatNumberShort(nextMilestone)}.`,
  ...
  if (!hasAnnualTarget) {
    ...
    if (contributionTarget > 1000) {
      suggestions.push({
        ...
        description: `Aim to invest £${formatNumberShort(contributionTarget)} this year to stay on track with your long-term goals.`,
        icon: '📅'
      });
    }
  }
