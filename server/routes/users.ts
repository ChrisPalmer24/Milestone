...
import { requireUser, AuthRequest } from "server/middleware/auth";
...
router.get("/me", requireUser, async (req: AuthRequest, res) => {
  try {
    const response = await requireTenant(req.tenant, async (tenant) => {
      const completeUser = await userService.getCompleteUser(tenant.id);
      return completeUser;
    });
    res.json(response);
  } catch (error) {
...
router.post("/change-password", async (req, res) => {
  try {
    ...
    const success = await userService.changePassword(userAccountId, currentPassword, newPassword);
    if (!success) {
      return res.status(400).json({ message: "Invalid current password or user account not found" });
    }
