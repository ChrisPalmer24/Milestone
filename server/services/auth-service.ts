...
import { parseTimeValue, timeToExpiryDate } from "../utils/time";
...
export async function generateRefreshToken(userId: string, deviceInfo: string): Promise<string> {
  ...
  return jwt.sign({ userId, familyId }, REFRESH_TOKEN_SECRET, {
    expiresIn: parseTimeValue(process.env.REFRESH_TOKEN_EXPIRY || "30d"),
  });
}

export async function verifyRefreshToken(token: string): Promise<{ userId: string; familyId: string }> {
...
export async function revokeRefreshTokenFamily(userId: string, familyId: string): Promise<void> {
  await db
    ...
    .where(
      and(
        eq(refreshTokens.userAccountId, userId),
      ...
      if (refreshToken) {
        try {
          ...
          const existingToken = await db.query.refreshTokens.findFirst({
            where: and(
              eq(refreshTokens.userAccountId, decoded.userId),
          ...
          const newRefreshToken = await generateRefreshToken(decoded.userId, userAgent || "unknown");
